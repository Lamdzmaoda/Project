...
@RequestMapping(
    "/my-learning") // Định nghĩa đường dẫn gốc cho các API trong class này là /permissions
...
public class UserLearningController {
  ...
  ApiResponse<List<LessonResponse>> getLessonByChapterId(
  ...
  ApiResponse<List<ChapterResponse>> getChapterByLanguageName(
  ...
  ApiResponse<LearingProgressResponse> learingProgressResponseApiResponse(
  ...
  ApiResponse<CodeResponse> submitPractice(@RequestBody PracticeSubmitRequest request) {
    return ApiResponse.<CodeResponse>builder()
        .result(practiceService.submit(request.getProblemId(), request.getCode()))
        .build();
  }
  ...
  ApiResponse<Void> resetProgress(String languageName) {
    learningProgressService.resetProgress(languageName);
