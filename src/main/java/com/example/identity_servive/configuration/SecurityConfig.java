...
@EnableMethodSecurity // Cho phép dùng @PreAuthorize trên các hàm trong Controller/Service để phân
// quyền chi tiết
public class SecurityConfig {
  ...
  private final String[] PUBLIC_ENDPOINTS = {
    ...
    "/piston", // Cho phép chính xác /piston
    "/piston/**",
            ...
            oauth2
                .jwt(
                    jwtConfigurer ->
                        jwtConfigurer
                            .decoder(
                                customJwtDecoder()) // Sử dụng bộ giải mã CustomJwtDecoder của mình
                            .jwtAuthenticationConverter(
                                jwtAuthenticationConverter())) // Chuyển đổi thông tin quyền hạn
                .authenticationEntryPoint(
                    new JwtAuthenticationEntryPoint())); // Xử lý lỗi khi Token sai/thiếu

    // 4. Tắt CSRF vì ứng dụng này là REST API dùng Token, không dùng Session/Cookie nên không cần
    // bảo vệ CSRF
  ...
  JwtAuthenticationConverter jwtAuthenticationConverter() {
    ...
    // Xóa tiền tố "SCOPE_" mặc định của Spring để dùng trực tiếp tên Role (VD: ADMIN thay vì
    // SCOPE_ADMIN)
