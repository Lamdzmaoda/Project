...
import com.example.identity_servive.dto.request.AuthRequest.*;
...
public class AuthenticationService {
  ...
  InvalidatedTokenRepository
      invalidatedTokenRepository; // Repository quản lý Token đã bị vô hiệu hóa
  ...
  @NonFinal
  @Value("${jwt.refreshable-duration}") // Thời gian tối đa có thể Refresh Token
  protected long REFRESHABLE_DURATION;

  @NonFinal
  @Value("${jwt.outbound.identity.client-id}")
  protected String CLIENT_ID;

  @NonFinal
  @Value("${jwt.outbound.identity.client-secret}")
  protected String CLIENT_SECRET;

  @NonFinal
  @Value("${jwt.outbound.identity.redirect-uri}")
            ...
            .expirationTime(
                new Date(
                    Instant.now()
                        ...
                        .toEpochMilli())) // Thời điểm hết hạn
                ...
                if (!CollectionUtils.isEmpty(role.getPermissions()))
                  role.getPermissions()
                      .forEach(
                          permission ->
                              stringJoiner.add(
                                  permission.getName()) // ví dụ: "read:user", "write:product"
                          );
              });
    }
