...
public class LearningProgressService {
    ...
    public void unlockNextChapter(User user, Chapter nextChapter) {
        ...
        lessonRepository.findAllByChapterIdAndStatusOrderByOrderIndexAsc(nextChapter.getId(), ContentStatus.ACTIVE)
    ...
    public void updateLessonToCompleted(User user, Lesson lesson) {
        ...
        UserLessonProgress progress = userLessonProgressRepository
                ...
                .orElseGet(() -> UserLessonProgress.builder()
                        .user(user).lesson(lesson).build());
        if(progress != null &&
                IsCompleted.TRUE.equals(progress.getCompletedStatus())){
            return;
        }
    ...
    public void updateChapterToCompleted(User user, Chapter chapter) {
        ...
        UserChapterProgress progress = userChapterProgressRepository
                .findByUserAndChapter(user, chapter)
                .orElseGet(() -> UserChapterProgress.builder()
                        .user(user).chapter(chapter).build());
        if(progress != null &&
                IsCompleted.TRUE.equals(progress.getCompletedStatus())){
            return;
        }
    ...
    public void initializeLearningProgressForLanguage(Language language, User user) {
        ...
        UserChapterProgress userChapterProgress = userChapterProgressRepository
                .findByUserAndChapter(user, firstChapter)
                .orElseGet(() -> UserChapterProgress.builder()
                        .user(user).chapter(firstChapter)
                        .completedStatus(IsCompleted.FALSE)
                        .build());
        ...
        lessonRepository.findAllByChapterIdAndStatusOrderByOrderIndexAsc(firstChapter.getId(), ContentStatus.ACTIVE)
                .forEach(lesson -> {
    ...
    public void completeLesson(User user, Lesson lesson, long totalXpGained) {
        ...
        enrollmentRepository.findByUserAndLanguage(user, lesson.getChapter().getLanguage())
                .ifPresent(enrollment -> {
                    // Cộng XP vào enrollment
                    enrollment.setCurrentXp(enrollment.getCurrentXp() + totalXpGained);

                    // Tính tổng số lesson ACTIVE trong language
                    long totalLessons = lessonRepository.countByChapterLanguageAndStatus(
                    ...
                    long totalInChapter = lessonRepository.countByChapterIdAndStatus
                            (lesson.getChapter().getId(), ContentStatus.ACTIVE);
                    long completedInChapter = userLessonProgressRepository.countByUserAndLesson_ChapterAndCompletedStatus
        ...
        long currentXp = enrollmentRepository.findByUserAndLanguage(user, lesson.getChapter().getLanguage())
                .map(Enrollment::getCurrentXp)
                .orElse(0L);
        Chapter nextChapter = chapterRepository.findFirstByLanguageAndStatusAndOrderIndexGreaterThanOrderByOrderIndexAsc(
                lesson.getChapter().getLanguage(), ContentStatus.ACTIVE, lesson.getChapter().getOrderIndex())
                .orElse(null);
        if(nextChapter == null) return;
        boolean alreadyUnlocked = userChapterProgressRepository.findByUserAndChapter(user, nextChapter)
    ...
    @Transactional
    public void restProgress(String languageName){
        ...
        enrollmentRepository.findByUserAndLanguage(user, language)
                .ifPresent(enrollment -> {
