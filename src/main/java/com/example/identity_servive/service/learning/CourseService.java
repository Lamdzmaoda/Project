...
public class CourseService {
  ...
  public StepResponse createStep(StepRequest stepRequest) {
    ...
    var lesson =
        lessonRepository
            .findByIdAndLessonType(stepRequest.getLessonId(), LessonType.LEARN)
  ...
  public ProblemResponse createProblem(ProblemRequest request) {
    Lesson lesson =
        lessonRepository
            .findByIdAndLessonType(request.getLessonId(), LessonType.PRACTICE)
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED));
  ...
  public ChapterResponse createChapter(ChapterRequest chapterRequest) {
    ...
    var language =
        languageRepository
            .findById(chapterRequest.getLanguageName())
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED));
  ...
  public StepResponse getStepById(String stepId) {
    return courseMapper.toStepResponse(
        stepRepository
            .findByIdAndStatus(stepId, ContentStatus.ACTIVE)
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED)));
  }

  @PreAuthorize("hasRole('USER')")
  public ProblemResponse getProblemById(String problemId) {
    return courseMapper.toProblemResponse(
        problemRepository
            .findByIdAndStatus(problemId, ContentStatus.ACTIVE)
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED)));
  }

  @PreAuthorize("hasRole('USER')")
  public LessonResponse getLessonById(String lessonId) {
    return courseMapper.toLessonResponse(
        lessonRepository
            .findByIdAndStatus(lessonId, ContentStatus.ACTIVE)
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED)));
  }

  @PreAuthorize("hasRole('USER')")
  public ChapterResponse getChapterById(String chapterId) {
    return courseMapper.toChapterResponse(
        chapterRepository
            .findByIdAndStatus(chapterId, ContentStatus.ACTIVE)
            .orElseThrow(() -> new AppException(ErrorCode.ID_NOT_EXISTED)));
  }

  @PreAuthorize("hasRole('USER')")
  public LanguageResponse getLanguageById(String languageName) {
    return courseMapper.toLanguageResponse(
        languageRepository
  ...
  public List<StepResponse> getStepByLesson(String lessonId) {
    var steps =
        stepRepository.findAllByLessonIdAndStatusOrderByOrderIndexAsc(
            lessonId, ContentStatus.ACTIVE);
  ...
  public List<ProblemResponse> getProblemByLesson(String lessonId) {
    ...
    return problemRepository
        .findAllByLessonIdAndStatusOrderByOrderIndexAsc(lessonId, ContentStatus.ACTIVE)
        .stream()
        .map(courseMapper::toProblemResponse)
        .toList();
  }
  ...
  public List<LessonResponse> getLessonByChapter(String chapterId) {
    var lessons =
        lessonRepository.findAllByChapterIdAndStatusOrderByOrderIndexAsc(
            chapterId, ContentStatus.ACTIVE);
  ...
  public List<ChapterResponse> getChapterByLanguage(String languageName) {
    var chapters =
        chapterRepository.findAllByLanguageNameAndStatusOrderByOrderIndexAsc(
            languageName, ContentStatus.ACTIVE);
  ...
  public List<ChapterResponse> getAllChaptersByLanguageForAdmin(String languageName) {
    return chapterRepository.findAllByLanguageNameOrderByOrderIndexAsc(languageName).stream()
  ...
  public List<UserChapterResponse> getMyLearning(String languageName) {
    ...
    return chapters.stream()
        .map(
            chapter -> {
              var chapterProgress =
                  userChapterProgressRepository.findByUserAndChapter(user, chapter).orElse(null);
              var lessons =
                  lessonRepository.findAllByChapterIdAndStatusOrderByOrderIndexAsc(
                      chapter.getId(), ContentStatus.ACTIVE);

              List<UserLessonResponse> lessonResponses =
                  lessons.stream()
                      .map(
                          lesson -> {
                            ...
                            return UserLessonResponse.builder()
                                ...
                                .lockedStatus(
                                    lessonProgress != null
                                        ? lessonProgress.getLockedStatus()
                                        : IsLocked.TRUE_LOCKED)
                                .completedStatus(
                                    lessonProgress != null
                                        ? lessonProgress.getCompletedStatus()
                                        : IsCompleted.FALSE)
                                .steps(
                                    LessonType.LEARN.equals(lesson.getLessonType())
                                        ...
                                        : null)
                                .problems(
                                    LessonType.PRACTICE.equals(lesson.getLessonType())
              ...
              return UserChapterResponse.builder()
                  ...
                  .completedStatus(
                      chapterProgress != null
                          ? chapterProgress.getCompletedStatus()
                          : IsCompleted.FALSE)
                  .lockedStatus(
                      chapterProgress != null
                          ? chapterProgress.getLockedStatus()
                          : IsLocked.TRUE_LOCKED)
  ...
  public LearingProgressResponse getLearningProgress(String languageName) {
    ...
    Enrollment enrollment = enrollmentRepository.findByUserAndLanguage(user, language).orElse(null);

    long currentXp = enrollment != null ? enrollment.getCurrentXp() : 0;
    long totalXp = calculateLanguageXp(languageName);
    int totalLesson =
        lessonRepository.countByChapterLanguageAndStatus(language, ContentStatus.ACTIVE);
    int completeLesson =
        userLessonProgressRepository.countByUserAndLesson_Chapter_LanguageAndCompletedStatus(
            user, language, IsCompleted.TRUE);
    int completeChapter =
        userChapterProgressRepository.countByUserAndChapter_LanguageAndCompletedStatus(
            user, language, IsCompleted.TRUE);
