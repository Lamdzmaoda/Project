...
public interface ChapterRepository extends JpaRepository<Chapter, String> {
  ...
  Optional<Chapter> findFirstByLanguageAndStatusOrderByOrderIndexAsc(
      Language language, ContentStatus status);

  Optional<Chapter> findFirstByLanguageAndStatusAndOrderIndexGreaterThanOrderByOrderIndexAsc(
      Language Language, ContentStatus status, int orderIndex);

  // Sửa đổi các phương thức tìm kiếm theo Lesson để chỉ lấy Step ACTIVE
  @EntityGraph(attributePaths = {"lessons"})
  List<Chapter> findAllByLanguageNameAndStatusOrderByOrderIndexAsc(
      String languageName, ContentStatus status);
